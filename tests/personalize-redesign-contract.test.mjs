...
test('PX-001 purchase configuration is owner-scoped, preview-guarded, lock-aware, server-priced, and locally optimistic', async () => {
  ...
  assert.match(purchasePanel, /setSelectedValue\(nextValue\)[\s\S]*startTransition/)
  assert.match(purchasePanel, /void onChange\(nextValue\)/)
  assert.doesNotMatch(purchasePanel, /disabled=\{selectionPending\}|useOptimistic/)
  assert.match(purchasePanel, /selectedValue === 'supreme'/)
  assert.match(page, /onChange=\{handleEditionChange\}/)
  assert.match(page, /queuedPurchaseBookTypeRef/)
  assert.match(page, /while \(queuedPurchaseBookTypeRef\.current\)/)
  assert.match(page, /setBookType\(nextPackageType\)/)
  assert.match(page, /confirmedPurchaseBookTypeRef/)
