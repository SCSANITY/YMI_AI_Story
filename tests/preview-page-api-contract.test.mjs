...
test('structured Preview signing preserves authorization and no-store responses', async () => {
  ...
  assert.match(source, /job\.status === ['"]running['"][\s\S]*status: ['"]pending['"][\s\S]*202/)
  assert.doesNotMatch(source, /Preview not ready['"], 400/)
...
test('structured Personalize pages leave variant, share, cart, and Reader contracts intact', async () => {
  ...
  assert.match(jobsService, /response\.status === 202[\s\S]*return null/)
  ...
  assert.match(previewController, /setPreviewBookPresentation\(\(current\) => mergePreviewPresentation\(current, assets\.presentation, renew\)\)/)
  assert.match(previewController, /const job = await getPreviewJobState\(jobId, customerId \?\? null\)/)
  assert.match(previewController, /const assets = job\.assets \? resolvePreviewDisplayAssets\(job\.assets\) : null/)
  assert.doesNotMatch(previewController, /await getJob\(|await getPreviewPageAssets\(/)
  assert.match(personalize, /bookPresentation=\{visiblePreviewPresentation\}/)
  assert.match(personalize, /createPreviewVersion\(/)
  assert.doesNotMatch(personalize, /commitPreviewVariant\(/)
