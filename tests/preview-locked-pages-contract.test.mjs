...
test('Customize Preview has no legacy second-page or whole-book preload authority', () => {
  ...
  assert.match(personalize, /Promise\.all\(\[\s*waitForImageDecode\(firstPreviewSpreadLeftUrl\),\s*waitForImageDecode\(firstPreviewSpreadRightUrl\),\s*\]\)/)
  assert.match(personalize, /firstPreviewSpreadReady=\{isFirstPreviewSpreadPairReady\}/)
  assert.match(personalize, /setDecodedFirstSpreadPairKey\(null\)/)
  assert.doesNotMatch(personalize, /previewFirstSpreadPresentation|getTemplatePreviewFirstSpreadDisplayUrls/)

  const pageContent = read('components/personalize/PreviewBookPageContent.tsx')
  assert.match(pageContent, /isWaitingForFirstSpreadPair/)
  assert.match(pageContent, /firstPreviewSpreadReady/)
  assert.doesNotMatch(pageContent, /FirstSpreadUnderlay|isGeneratingWithUnderlay/)
