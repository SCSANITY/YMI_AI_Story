...
test('professional reply routing accepts only current customer-facing aliases', async () => {
  ...
  assert.doesNotMatch(support, /support\\\+|legacyMatch|ticket-/)
  assert.doesNotMatch(kol, /partners\\\+|legacyMatch|collab-/)

  assert.match(processor, /\.eq\(['"]reply_alias['"], routedAddress\.replyAlias\)/g)
  assert.doesNotMatch(processor, /routedAddress\.replyToken|routedAddress\.(?:ticketCode|leadCode)/)
