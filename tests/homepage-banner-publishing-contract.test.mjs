...
test('Home renders only the three published banner anchors without hard-coded campaign art', async () => {
  ...
  assert.match(banner, /getImageProps\([\s\S]*src: banner\.mobile\.src[\s\S]*width: banner\.mobile\.width/)
  assert.match(banner, /<source[\s\S]*srcSet=\{mobileSrcSet\}[\s\S]*width=\{banner\.mobile\.width\}/)
  assert.match(banner, /srcSet=\{desktopSrcSet\}/)
